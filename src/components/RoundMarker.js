...
const RoundMarker = ({ business, isSelected, onPress }) => {
  ...
  const iconFillColor = business.visited
    ? iconFillVisited
    : business.iconHexColor;

  const styles = StyleSheet.create({
    container: {
      borderRadius: 999,
      borderWidth: 1,
      borderColor: !business.visited
        ? business.iconHexColor || ColorPalette.getHexColorByName('yelpRed')
        : `rgba(0,0,0,0.20)`,
      ...
      backgroundColor: business.visited
        ? business.iconHexColor || ColorPalette.getHexColorByName('yelpRed')
        : `#fff`,
  ...
  const handleAnimation = () => {
    scaledUp.value = isSelected
      ? withSpring(scaledUp.value * 1.25, springConfig)
  ...
  return (
    <Marker
      ...
      onPress={onPress}
    >
      <Animated.View style={animatedStyle} entering={ZoomIn} exiting={ZoomOut}>
